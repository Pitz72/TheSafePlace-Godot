...
func _on_test_combat_button_pressed():
	print("🐛 DEBUG: Avvio combattimento di test...")
	if CombatManager:
		CombatManager.start_combat("giant_rat_01")
	else:
		print("❌ DEBUG ERRORE: CombatManager non trovato.")
		add_log_message("[color=red]ERRORE: CombatManager non trovato.[/color]")
